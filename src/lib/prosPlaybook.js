...
export function buildProsKnowledgeText() {
  const lines = [
    '## AiBhive Pros (field service HQ)',
    `Pros lives at https://www.manydoorsai.com/pros and the live HQ app at ${PROS_APP_URL}.`,
    'Trades: HVAC, plumbing, electrical, pool, property maintenance, fiber/low voltage.',
    'Diagnose is the Android field app. Techs capture voice/photo notes; managers dispatch from Pros HQ.',
    'Voice demos: use triage_maintenance for resident tickets, then lookup_pros_playbook for tech-side advice.',
    '',
  ];
  for (const t of PROS_TRADE_PLAYBOOKS) {
    lines.push(`### ${t.name}`);
    lines.push(t.summary);
    lines.push(`First questions: ${t.firstQuestions.join(' ')}`);
    lines.push(`Self-help: ${t.selfHelp.join(' ')}`);
    lines.push(`Escalate: ${t.escalate.join(' ')}`);
    lines.push('');
  }
  return lines.join('\n');
}
